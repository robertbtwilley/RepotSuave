...
//           	Zwrite Off
            ...
            {           
                ...
                float ortho = 1;
                float persp = 7;												
				//--------  geometry scale technique --------------
				float3 vertexOutline = fixed3(0,0,0);				
				//#ifdef OUTLINE_BASIC
					vertexOutline = v.vertex.xyz + v.normal * _Outline;
				//#endif
				//#ifdef OUTLINE_ADVANCED
				//	o.uv = TRANSFORM_TEX(v.texcoord, _OutlineTex); // this allows you to offset uvs and such
				//	vertexOutline.x = v.vertex.x + v.normal.x *_OutlineX;
    ...
    //            #ifdef OUTLINE_BASIC
	   //             #ifdef ORTHO_OFF
	   //             	o.pos.xy += offset * (_Outline / persp) *.01;
	   //             #endif
	   //             #ifdef ORTHO_ON
	   //             	o.pos.xy += offset * (_Outline / ortho)*.01;// fix for othographic
	   //         	#endif
    //        	#endif
    ...
    //            #ifdef OUTLINE_ADVANCED
	   //             o.uv = TRANSFORM_TEX (v.texcoord, _OutlineTex); // this allows you to offset uvs and such
	   //             #ifdef ORTHO_OFF		                
		  //              o.pos.x += offset.x * (_OutlineX / persp )*.01;
		  //              o.pos.y += offset.y * (_OutlineY / persp )*.01;
		  //          #endif
		  //          #ifdef ORTHO_ON
	   //             	o.pos.x += offset.x  * (_OutlineX / ortho )*.01;// fix for orthographic
	   //             	o.pos.y += offset.y  * (_OutlineY / ortho )*.01;// fix for orthographic
	   //         	#endif
				//#endif
				...
				////#ifdef OUTLINE_BASIC
				//		pos.xyz = normalize(v.vertex) + v.normal *_Outline;
    //        	//#endif
    ...
    //            #ifdef OUTLINE_ADVANCED
	   ...
	   //         #endif				
				...
				//
				#ifdef OUTLINE_ADVANCED			
					o.pos.x += offset.x * _OutlineX;
					o.pos.y += offset.y * _OutlineY;					
				#endif
