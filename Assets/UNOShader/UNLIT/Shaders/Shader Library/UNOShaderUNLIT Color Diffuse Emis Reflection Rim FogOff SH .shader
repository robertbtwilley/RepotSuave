...
//Version=1.7
Shader"UNOShader/_Library/UNLIT/UNOShaderUNLIT Color Diffuse Emis Reflection Rim FogOff SH "
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
			...
			//__________________________________ Emission _____________________________________
				float2 uvEmission = TRANSFORM_TEX (i.uv, _EmissionMap); // this allows you to offset uvs and such
				uvEmission = mul(_EmissionMapMatrix, float4(uvEmission,0,1)).xy; // this allows you to rotate uvs and such with script help
				float4 T_Emission = tex2D (_EmissionMap, uvEmission) * _EmissionColor;
